...
**Model catalog:** `configs/auto-route-defaults.yaml` is the canonical source for model IDs. Resolve it beside the installed skill (or from `COUNCIL_MODEL_CONFIG` when set), load it before routing, and fail with a clear configuration error if a required provider tier is missing. Provider detection, tier matching, and Chairman auto-selection must use this catalog; never substitute model IDs from this protocol text.
...
1. Run the detection script via Bash: `bash ~/.claude/skills/council/scripts/detect-providers.sh`. It reads the adjacent canonical model catalog automatically.

...
1. `~/.codex/skills/council/` (`agents/` plus `configs/auto-route-defaults.yaml`)
...
1. **Explicit override**: `--chairman <name>` was passed (provider tag — `anthropic`, `openai`, `google`, `ollama`, `nvidia_nim`, `cursor_cli` — or an exact catalog model ID).
